...
export default function Post({
  ...
  return (
    <motion.div
      ...
      className="flex items-center rounded-lg justify-between w-full h-min p-8 px-10 shadow"
    >
      <div className="flex-1">
        <span className="text-[#727272]">{moment(data.time).format('MMM DD, YYYY')}</span>
        <h1 className="text-2xl">{data.name}</h1>
        <p className="text-[#727272] mt-2 abstract w-full">
          {(() => {
      ...
      <div className="dropdown dropdown-end">
        ...
        >
          <Icon
            icon="uil:ellipsis-v"
            className="text-[#727272] w-5 h-5 mt-[1px]"
          />
        </label>
